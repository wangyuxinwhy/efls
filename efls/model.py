...
class Efls(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer('efls_dummy_token_ids', torch.tensor(list(range(99-num_semantic_tokens, 99)), dtype=torch.long))

    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> dict[str, torch.Tensor]:
        input_ids = torch.cat([self.efls_dummy_token_ids.unsqueeze(0).expand(input_ids.shape[0], -1), input_ids], dim=1)
        attention_mask = torch.cat([torch.full_like(self.efls_dummy_token_ids, 1).unsqueeze(0).expand(attention_mask.shape[0], -1), attention_mask], dim=1)
    ...
    def save_pretrained(self, output_dir: Path | str):
        ...
        torch.save(self.projector.state_dict(), output_dir / self.EFLS_PROJECTOR_STATE_NAME)
...
class EmbeddingFromLanguageModel(nn.Module):
    ...
    def __init__(self, encoder: PreTrainedModel, decoder: PreTrainedModel, projection_size: int = 300, num_efls_tokens: int = 10):
        super().__init__()
        self.efls = Efls(encoder, projection_size, num_efls_tokens)
        self.num_tokens = num_efls_tokens
        self.register_buffer('efls_dummy_token_ids', torch.tensor([1] * num_efls_tokens, dtype=torch.long))
    ...
    def forward(
        ...
        decoder_input_ids = torch.cat([self.efls_dummy_token_ids.unsqueeze(0).expand(decoder_input_ids.shape[0], -1), decoder_input_ids], dim=1)
        labels = torch.cat([torch.full_like(self.efls_dummy_token_ids, -100).unsqueeze(0).expand(labels.shape[0], -1), labels], dim=1)
